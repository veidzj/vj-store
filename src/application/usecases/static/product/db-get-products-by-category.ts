...
export class DbGetProductsByCategory implements GetProductsByCategory {
  ...
  public getByCategory = async(category: string, page: number, limit: number, sortBy?: string): Promise<Product[]> => {
    const categoryExists = await this.checkCategoryByNameRepository.checkByName(category)
    if (!categoryExists) {
      throw new CategoryNotFoundError()
    }
    const products = await this.getProductsByCategoryRepository.getByCategory(category, page, limit, sortBy)
    return products
  }
}
